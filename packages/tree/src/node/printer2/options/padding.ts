    ...
    for (const node of nodes) {
      if (node.syntax.name === `nil`) {
        ...
        if (node.syntax.name === `imaginary`) {
          padding.after(node.start)
          padding.before(node.end!)
        } else {
          // pad relevant fractions of node content ("generic" way)
          for (let i = 0; i < node.relevant.length; i++) {
            const relevant = node.relevant[i]
            if (relevant === undefined) debugger

            if (isArray(relevant)) {
              const _range = relevant as [number, number]

              padding.after(_range[0])
              padding.before(_range[1])
            } else {
              padding.both(relevant as number)
            }
          }
        }
      }
